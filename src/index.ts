import { get, getGenres, random, release, search } from "./scraper/index.js";
import type {
  EpisodeMetadata,
  Genre,
  HentaiMetadata,
...
export default class NekoBocc {
   ...
   */
  release(page?: number): Promise<HentaiRelease[] | IError> {
    return release(page);
  }
   ...
   */
  search(query: string): Promise<HentaiRelease[] | IError> {
    if (!query.trim()) {
